...
public class LoginFragment extends Fragment {
    ...
    private void loginUser() {
        ...
        if (TextUtils.isEmpty(Email)){
            ...
            MyActivity.appPreference.showToast("Password  may be at least 6 characters long.");
        } else {
            Call<User> userCall = MyActivity.serviceApi.doLogin(Email, Password);
            userCall.enqueue(new Callback<User>() {
                @Override
                public void onResponse(Call<User> call, Response<User> response) {
                    if (response.body().getResponse().equals("data")){
                        MyActivity.appPreference.setLoginStatus(true); // set login status in sharedPreference
                        ...
                        MyActivity.appPreference.showToast("Error. Login Failed");
                        emailInput.setText("");
